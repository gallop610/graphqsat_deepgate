...
def build_argparser():
    ...
    parser.add_argument(
        "--model_to_best_eval_path",
        help="Path to the best model for evaluation",
        type=str,
    )
    ...
    if args.dump_timings_path:
        ...
        prop_file_name = (
            os.path.join(
                args.dump_timings_path,
                args.eval_problems_paths.replace("/", "_")
                + f"_cap_{args.test_time_max_decisions_allowed}_propagations",
            )
            + ".pkl"
        )
        with open(prop_file_name, "wb") as f:
